...
/**
 * Desktop implementation of PreferencesDataStore using file-based storage.
 *
...
actual class PreferencesDataStore actual constructor(name: String) {
    private val dataStore: DataStore<Preferences> =
        PreferenceDataStoreFactory.createWithPath(
            produceFile = {
                val dataDir = File(System.getProperty("user.home"), ".kmp-datastore")
                if (!dataDir.exists()) {
                    dataDir.mkdirs()
                }
                File(dataDir, "$name.preferences_pb").absolutePath.toPath()
            },
        )
